 ...
 * @since pcap4j 0.9.12
 */
public final class TcpPacket extends AbstractPacket implements TransportPacket, PacketDissector {
    ...
    public void dissect(PacketEntry entry) {
        ...
        if(header.getAck()) sb.append("ACK, ");
        if(header.getFin()) sb.append("FIN, ");
        if(header.getRst()) sb.append("RST, ");
        if(header.getPsh()) sb.append("PSH, ");
        if(header.getUrg()) sb.append("URG, ");
        ...
        entry.info = String.format("%d -> %d [%s] Seq=%d Ack=%d Win=%d Len=%d",
                header.getSrcPort().valueAsInt(),
                header.getDstPort().valueAsInt(),
                flags.substring(0, flags.length() - 2),
                header.getSequenceNumberAsLong(),
                header.getAcknowledgmentNumberAsLong(),
    ...
    public static final class TcpHeader extends AbstractHeader implements TransportHeader {
        ...
        private TcpHeader(byte[] rawData, int offset, int length) throws IllegalRawDataException {
            ...
            try {
                ...
                Log.e(TAG, "Exception occurred during analyzing TCP options: ", e);
            }
